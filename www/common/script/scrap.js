...
VIABOOK.prototype = {
	...
	init		:	function(){
		...
		*/
		...
		this.cache['body'] = articleContent;		
		this.cache["enconvert"] = this.convert(this.cache['body']); 
		//document.getElementById("preview").innerHTML = this.cache['enconvert'];
	...
	makeSimpleLayer 	: 	function(){
		...
		//var preview = dom.makeEle("div",{id : "preview"});
		//var previewTag = divTag.appendChild(preview);
	...
	//	statusTag 	+= '	Have Saved content at viabook';
		...
		var prepare_content ='<ul style="width:220px; margin:0; padding:0; overflow:hidden; list-style:none;">'
		prepare_content +='<li>'
		prepare_content +='<div style="float:left; width:220px; padding:2px 0; margin:0; overflow:hidden;">';
		...
		prepare_content +='Content URL';
		prepare_content +='</label>';
		prepare_content +='<span>';
		prepare_content +='<input type="text" value="'+document.location+'" id="s_url" style="width:151px; height:20px; padding:3px 0 0 2px; border:1px solid #ccc; color:#666; font-size:12px;line-hegiht:12px;background-color:#fff;">';
		prepare_content +='</span>';
		prepare_content +='</div>';
		prepare_content +='</li>';
		prepare_content +='</ul>';
		...
		prepare_content +='<span id="viabook_user"></span> log in viaBook ';
		...
		prepare_content +='</div>';
		prepare_content +='<input type="button" id="save_btn" value="save to viabook" style="border:1px solid #D6D6D6; padding:2px 5px; color:#666; font-size:12px">';
		prepare_content +='</div>';


	   
	    var loginTag ='<ul style="width:220px; margin:0; padding:0; overflow:hidden; list-style:none;">';
		loginTag +='	<li>';
		loginTag +='<div style="float:left; width:220px; padding:2px 0; margin:0; overflow:hidden;">';
		...
		loginTag +='<span style="display:block; float:left; width:155px;">';
		loginTag +='<input type="password" name="" id="v_pass" style="width:151px;background-color:#fff; height:20px; padding:3px 0 0 2px; border:1px solid #ccc; color:#666; font-size:12px"></span>';
		loginTag +='</div>';
		loginTag +='</li>';
		loginTag +='</ul>';
		...
		document.getElementById("save_btn").onclick = function(){
			via.postServer(via.cache["enconvert"]);
		}
	...
	convert		:	function(element){
		...
		for(var i=0;i<img.length;i++){
			...
			if(img[i].getAttribute("width")<30 || img[i].getAttribute("height")<30){
				try{
	...
	postServer : function(data){
		...
		this.showLayer("status_layer");
		document.getElementById("text_msg").innerHTML="Have saved at viabook";
		if(this.debug){
			dom.setCss(document.getElementById("loadingDiv"),{"display" : "none"});
